...
df_clean["price_change_pct"] = (
    df_clean.groupby(["commodity", "market"])["price"].pct_change() * 100
)

# Define target: High volatility = |price change| > 20%
VOLATILITY_THRESHOLD = 20
df_clean["high_volatility"] = (
    abs(df_clean["price_change_pct"]) > VOLATILITY_THRESHOLD
).astype(int)
...
df_features["is_maha_season"] = (
    df_features["month"].isin([10, 11, 12, 1, 2]).astype(int)
)
...
df_features["months_since_start"] = (
    (df_features["date"] - min_date).dt.days / 30.44
).astype(int)
...
def create_lag_features(group):
    ...
    group["rolling_volatility_3"] = (
        group["price_change_pct"].abs().rolling(window=3, min_periods=1).mean()
    )

    # Price momentum (current vs 3-month average)
    group["price_momentum"] = (
        (group["price"] - group["rolling_mean_3"])
        / (group["rolling_mean_3"] + 0.001)
        * 100
    )
...
df_features = df_features.groupby(["commodity", "market"], group_keys=False).apply(
    create_lag_features
)
...
df_features["z_score"] = (df_features["price"] - df_features["rolling_mean_6"]) / (
    df_features["rolling_std_6"] + 0.001
)

# Price vs annual average
annual_avg = df_features.groupby(["commodity", "year"])["price"].transform("mean")
df_features["price_vs_annual_avg"] = (
    (df_features["price"] - annual_avg) / (annual_avg + 0.001) * 100
)

# Price vs category average
category_avg = df_features.groupby(["category", "year", "month"])["price"].transform(
    "mean"
)
df_features["price_vs_category_avg"] = (
    (df_features["price"] - category_avg) / (category_avg + 0.001) * 100
)
...
df_features["distance_from_colombo"] = haversine_distance(
    df_features["latitude"], df_features["longitude"], colombo_lat, colombo_lon
)

df_features["is_conflict_region"] = (
    df_features["admin1"].isin(["Northern", "Eastern"]).astype(int)
)
...
df_features["is_essential"] = (
    df_features["commodity"].isin(essential_commodities).astype(int)
)
...
print(
    f"Best validation AUC: {catboost_model.get_best_score()['validation']['AUC']:.4f}"
)
...
classification_rep = classification_report(
    y_test, y_test_pred, target_names=["Stable", "Volatile"], output_dict=True
)
...
shap_values = catboost_model.get_feature_importance(
    Pool(X_sample, cat_features=cat_feature_indices), type="ShapValues"
)
